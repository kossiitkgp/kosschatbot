...
def main(query):
    ...
    result_list = list()
    while True:
        base_url = "http://stackoverflow.com/search?page={}&pagesize=50&tab=relevance&q={}".format(
            page_no, formatted_query)
        ...
        for result in results:
            flag = False
            # getting accepted answered questions
            accepted_answered = result.find(
                'div', {'class', 'status answered-accepted'})
            answered = result.find('div', {'class', 'status answered'})
            if accepted_answered is not None:
            ...
            subtitle = "This question received {} answers and {} votes.".format(
                no_of_ans, no_of_votes)
            result_dict = {'item_url': ques_url, 'subtitle': subtitle, 'title': title, 'buttons': [
                {"type": "web_url", "title": "View", "url": ques_url}, {"type": "element_share"}]}
